                ...
                */
                ...
                if(subtitles.hasOwnProperty(subtitlesId)) {
                    video.addEventListener("loadedmetadata", function() { 
						track = this.addTextTrack("subtitles", subtitles[subtitlesId].name, subtitles[subtitlesId].lang); 
                        track.mode = "showing";
                        for(var i=0; i < subtitles[subtitlesId].tracks.length; i++) {
                            var tempText = subtitles[subtitlesId].tracks[i].text;
                            var regex = /<br\s*[\/]?>/gi;
                            tempText = tempText.replace(regex, "\r\n");
                            track.addCue(new VTTCue(subtitles[subtitlesId].tracks[i].start, subtitles[subtitlesId].tracks[i].end, tempText));
                        }
                    });
				}
            });
        ...
        if (!notstartup) {
            $(".video-player").each(function () {
                ...
                */
                ...
                if(subtitles.hasOwnProperty(subtitlesId)) {
                    html += '<div class="subtitles">';
                    for(var i=0; i < subtitles[subtitlesId].tracks.length; i++) {
                        html += '<p data-start="'+subtitles[subtitlesId].tracks[i].start+'" data-end="'+subtitles[subtitlesId].tracks[i].end+'" style="display:none;">'+subtitles[subtitlesId].tracks[i].text+'</p>';
                    }
                    html += '</div>';
                }
