(function(Popup, $, undefined) {
    ...
    Popup.ShowVideo = function(source, title, options) {
        ...
        if(!settings.showFullScreen) {
            video.addClass("fullscreen-inactive");
        }
    ...
    $(function() {
        $(".new-popup-link").each(function() {
            ...
            var pathPrefix = $this.attr("data-path-prefix");
    ...
    VideoPlayer.InitPlayers = function(selector) {
        ...
        element.each(function(index) {
            ...
            var settings = {
                ...
                pathPrefix: (typeof $this.attr("data-path-prefix") === "undefined" ? "" : $this.attr("data-path-prefix"))
            };
            ...
            var $video = $('<video id="video-' + index + '" />').attr("src", settings.pathPrefix + "files/" + source + ".mp4").appendTo(videoContainer).data("parent", $this);
            var video = $video[0];
            var videoDuration = 0;
            ...
            var playBtn = $('<span class="new-player-play-icon"><img src="' + settings.pathPrefix + 'images/new-player-play.png" /></span>').appendTo(playPauseControl);
            var pauseBtn = $('<span class="new-player-pause-icon"><img src="' + settings.pathPrefix + 'images/new-player-pause.png" /></span>').appendTo(playPauseControl);
            ...
            if(settings.audio && !navigator.userAgent.match(/iPhone|iPad|iPod/i)) {
                ...
                var volumeIcon = $('<div class="volume-icon"><img src="' + settings.pathPrefix + 'images/new-player-audio.png" /></div>').appendTo(volumeControl).on("click", function() {
            ...
            var progressSubtitlesLocations = $('<div class="new-player-progress-subtitle-locations"></div>').appendTo(progressBackground);
            ...
            if(settings.fullscreen) {
                var fullscreenBtn = $(' <div class="new-player-fullscreen"><img class="open-fullscreen" src="' + settings.pathPrefix + 'images/new-player-fullscreen.png" /><img class="close-fullscreen" src="' + settings.pathPrefix + 'images/new-player-close-fullscreen.png" /></div>').appendTo(controlsContainer);
            }
            var accessabilityBtn = $(' <div class="new-player-accessability"><img src="' + settings.pathPrefix + 'images/new-player-accessability.png" /></div>').appendTo(controlsContainer);
            ...
            video.addEventListener("play", function() {
                var tmpVideo = $(".new-player[playing]").removeAttr("playing").find("video");
            ...
            video.addEventListener("loadeddata", function() {
                ...
                if(subtitles.hasOwnProperty(subtitlesId)) {
                    for(var i = 0; i < subtitles[subtitlesId].tracks.length; i++) {
                        var pointLeft = subtitles[subtitlesId].tracks[i].start / videoDuration * 100;
                        $('<div class="new-player-progress-subtitle-point" style="left:' + pointLeft + '%;"></div>').appendTo(progressSubtitlesLocations);
                    }
                }
                ProgressVideo();
            }, false);
...
function EnableSubtitleHelp() {
    $("body").addClass("enable-subtitle-help");
}